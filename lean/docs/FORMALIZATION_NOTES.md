...
# Informalidad y Distribucion de Riqueza: Un Modelo de Agentes Heterogeneos con Oferta Laboral Endogena (Anexo Matematico)

## Source Version

- Paper: *Informalidad y Distribucion de Riqueza: Un Modelo de Agentes Heterogeneos con Oferta Laboral Endogena (Anexo Matematico)*
- Authors: John Svante Barraza Ratachi and Enzo Andres Nevado Martinez
- Version formalized: Anexo Matematico v10 ARz DebtPrem, Junio 2026 (aiyagari-2firms-peru)
- Official URL: https://github.com/johnbarraza/aiyagari-2firms-peru
- Public PDF: https://github.com/johnbarraza/aiyagari-2firms-peru
...
- Implementation theorem file: `BN26InformalityWealthPeru/MainTheorems.lean`
- Human-facing theorem file: `BN26InformalityWealthPeru/PaperInterface.lean`
- Machine-readable status source: `BN26InformalityWealthPeru/status.json`
- Private outside-Lean proof plan: `BN26InformalityWealthPeru/docs/FORMALIZATION_PLAN.md`
- Final validation report: `BN26InformalityWealthPeru/FINAL_VALIDATION_REPORT.md`
- Dependency DAG: `BN26InformalityWealthPeru/docs/DependencyDAG.tex`
- Rendered DAG: `BN26InformalityWealthPeru/docs/DependencyDAG.pdf`
- LLM/source audit sidecars: `BN26InformalityWealthPeru/audit/*.json`
...
`python3 scripts/review_dashboard.py --paper BN26InformalityWealthPeru --statement-precheck`.
...
Then run `python3 scripts/review_dashboard.py --paper BN26InformalityWealthPeru
...
`python3 scripts/review_dashboard.py --paper BN26InformalityWealthPeru --precheck` only to
...
`python3 scripts/closeout_reuse_plan.py --paper BN26InformalityWealthPeru` and execute only its
current `next_action`.
...
`python3 scripts/review_dashboard.py --paper BN26InformalityWealthPeru --paper-coverage-precheck`
...
|---|---|---|---|---|
| Generador OU discretizado: filas suman cero (Section 2.2, Equation eq:Qz_construction, anexo_matematico.tex:97) | `paper_ou_generator_rows_sum_zeroSpec` -> `paper_ou_generator_rows_sum_zero` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
| Prima de deuda estrictamente decreciente en la productividad (Section 3.2, Equation eq:debtprem, anexo_matematico.tex:148) | `paper_debt_premium_decreasing_in_productivitySpec` -> `paper_debt_premium_decreasing_in_productivity` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
| Consumo CES optimo: ratio de demanda y deflactor (Section 3.5, Equation eq:ces_foc, anexo_matematico.tex:188) | `paper_ces_optimal_demand_ratioSpec` -> `paper_ces_optimal_demand_ratio` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
| Oferta laboral interior: condicion de primer orden y optimalidad global (Section 3.6, Equation eq:foc_F, anexo_matematico.tex:206) | `paper_interior_labor_first_order_conditionSpec` -> `paper_interior_labor_first_order_condition` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
| Caso vinculante KKT: monotonia estricta y casos de esquina (Section 3.6, Equation eq:kkt_binding, anexo_matematico.tex:211) | `paper_binding_labor_kkt_monotone_and_cornersSpec` -> `paper_binding_labor_kkt_monotone_and_corners` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
| Firma formal Cobb-Douglas CRS: CPO estaticas y agotamiento del producto (Section 4.1, Equation eq:formal_foc, anexo_matematico.tex:243) | `paper_formal_firm_first_order_conditionsSpec` -> `paper_formal_firm_first_order_conditions` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
| Firma informal: PMgL, demanda de capital y beneficio residual (Section 4.2, Equation eq:PiI, anexo_matematico.tex:268) | `paper_informal_firm_profit_exhaustionSpec` -> `paper_informal_firm_profit_exhaustion` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
| Vaciamiento del mercado de bien formal (Ley de Walras) (Section 5.2, anexo_matematico.tex:302) | `paper_walras_formal_goods_marketSpec` -> `paper_walras_formal_goods_market` | statement specification + proof stub | `PaperInterface.lean` | The transparent `...Spec : Prop` is the statement-audit target; the proof body is `by sorry`; raw-source-to-expanded-Spec judgment and premise provenance pending |
...
- [ ] Confirm `scripts/audit_conclusion_provenance.py --paper BN26InformalityWealthPeru` reports
...
- [ ] Update `status.json`, then run `python3 scripts/sync_paper_status.py
      --paper BN26InformalityWealthPeru`. Defer the unscoped aggregate/site sync to integration
      or release.
...
- [ ] After the report/DAG/status updates, run
      `python3 scripts/closeout_reuse_plan.py --paper BN26InformalityWealthPeru`, follow its
