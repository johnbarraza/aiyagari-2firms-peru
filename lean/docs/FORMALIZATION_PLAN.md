# Formalization Plan: Informalidad y Distribucion de Riqueza: Un Modelo de Agentes Heterogeneos con Oferta Laboral Endogena (Anexo Matematico)
...
- Source version / local files inspected: `anexo_matematico.tex` (SHA-256
  `2b7c3362...7929ef`) del paquete de replicacion `aiyagari-2firms-peru`; se
  contrastaron ademas el documento principal `INFORMALIDAD_RIQUEZA_HA_PERU.md`
  y el solver `model_main.m` del mismo paquete.
- Source/version mismatch notes: el anexo esta desactualizado respecto del solver
  en tres puntos (forma de la prima de deuda, forma de la barrera de acceso
  formal, y demanda estatica de capital informal sin el factor de precio), y es
  internamente inconsistente en un cuarto (pesos del agregador CES). El documento
  principal, a su vez, difiere del anexo y del solver en la formula del beneficio
  informal.
- Complete named-result ledger status: el anexo no numera proposiciones ni
  teoremas. El inventario normal se construyo sobre las ecuaciones numeradas que
  enuncian una afirmacion derivable: ocho en total.
- Formula sanity check:
  - Signs, constants, normalizations, quantifiers, domains: se detecto una
    normalizacion incorrecta en los pesos del agregador CES y un factor de precio
    faltante en la demanda de capital informal.
  - Density vs mass / likelihood-kernel representation issues: no aplica; la
    distribucion estacionaria entra solo como operador de agregacion.
  - Dependency map between named source results: las ocho filas son mutuamente
    independientes salvo que la identidad de vaciamiento consume las condiciones
    de ambas firmas.
  - Formula-bearing displayed claims that need derivation, not source-row
    assumptions: la identidad de vaciamiento del bien formal, que el anexo lista
    como condicion pero que en realidad se deriva de las demas.
- Named result sanity check:
  - Results that look correct as stated: generador OU, monotonia de la prima,
    condiciones de primer orden laborales, caso vinculante KKT, firma formal, y
    beneficio residual informal.
  - Suspected bugs, missing assumptions, or ambiguous wording: agregador CES,
    demanda de capital informal, e identidad de vaciamiento del bien formal.
- Source-proof fidelity ledger (`audit/source_proof_fidelity.json`):
  - Proof scopes reviewed by source locator and mathematical claim: hogares,
    firmas, equilibrio, y proceso de productividad.
  - Source proof defects, if any, with repair obligation and acceptance
    condition: cinco defectos registrados con obligacion de reparacion explicita.
  - Proof-only defects kept out of `Assumptions.lean`: `Assumptions.lean` esta
    vacio; ningun defecto se convirtio en supuesto.
- Shared-library reuse checkpoint:
  - Mathlib declarations/modules inspected: `Analysis.SpecialFunctions.Pow.Real`
    (potencias reales) y `Analysis.Convex.SpecificFunctions.Basic` (desigualdad
    de Bernoulli para exponentes reales).
  - Cslib declarations/modules inspected: ninguno relevante.
  - Optlib declarations/modules inspected: no disponible en este entorno.
  - Other potential upstream sources inspected: ninguno.
  - Upstream sources used or ported, with citation/provenance: solo Mathlib.
  - Existing `EconCSLib` declarations/modules inspected: ninguno aplicable; las
    filas son de analisis real elemental, no de teoria de mecanismos.
  - API chosen and near-misses: se descarto la desigualdad de Young a favor de
    Bernoulli para exponentes reales, que da el optimo con igualdad exacta.
  - Source-defined objects that will use reusable library definitions, and the
    planned paper-local semantic bridge/equivalence rows: ninguno; todas las
    filas se escriben con aritmetica real y potencias reales directamente.
- Proof strategy consequences:
  - Source proof route to follow: la del anexo, salvo en las tres filas con
    correccion de fuente.
  - Cleaner Lean route or reusable library route: Bernoulli para la optimalidad
    laboral global; combinacion lineal exacta para la identidad de Walras.
  - Major issues already reported to the user: los cinco defectos de la Seccion
    10 y 11 del reporte de validacion.
- Algorithmic complexity audit, when applicable: no aplica; ninguna fila es una
  afirmacion de tiempo de ejecucion.

## Source Inventory

- Definitions / formatted paper objects: proceso OU y su discretizacion,
  preferencias y restriccion presupuestaria del hogar, prima de deuda, barrera de
  acceso formal, ecuacion HJB, agregador CES, salarios efectivos por sector,
...
- Named lemmas / propositions / theorems / corollaries: ninguno; el anexo no
  numera resultados. El alcance normal se tomo como las ocho ecuaciones numeradas
  que enuncian una afirmacion derivable.
- Named assumptions / model conditions used by those results: ninguno declarado
  como supuesto ajeno; todas las hipotesis de las filas son condiciones de
  positividad y de rango de parametros que el propio anexo fija.
- Deep-only prose, standalone formulas, algorithms, figures, simulations, and
  computational examples: el esquema upwind implicito, la construccion de la
  matriz de transicion completa, los bucles de equilibrio, las grillas, y toda la
  seccion de calibracion quedan fuera del alcance normal y no generan filas.
...
|---|---|---|---|---|---|
| Generador OU discretizado: filas suman cero / Section 2.2, Equation eq:Qz_construction, anexo_matematico.tex:97 | `paper_ou_generator_rows_sum_zeroSpec` -> `paper_ou_generator_rows_sum_zero` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
| Prima de deuda estrictamente decreciente en la productividad / Section 3.2, Equation eq:debtprem, anexo_matematico.tex:148 | `paper_debt_premium_decreasing_in_productivitySpec` -> `paper_debt_premium_decreasing_in_productivity` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
| Consumo CES optimo: ratio de demanda y deflactor / Section 3.5, Equation eq:ces_foc, anexo_matematico.tex:188 | `paper_ces_optimal_demand_ratioSpec` -> `paper_ces_optimal_demand_ratio` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
| Oferta laboral interior: condicion de primer orden y optimalidad global / Section 3.6, Equation eq:foc_F, anexo_matematico.tex:206 | `paper_interior_labor_first_order_conditionSpec` -> `paper_interior_labor_first_order_condition` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
| Caso vinculante KKT: monotonia estricta y casos de esquina / Section 3.6, Equation eq:kkt_binding, anexo_matematico.tex:211 | `paper_binding_labor_kkt_monotone_and_cornersSpec` -> `paper_binding_labor_kkt_monotone_and_corners` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
| Firma formal Cobb-Douglas CRS: CPO estaticas y agotamiento del producto / Section 4.1, Equation eq:formal_foc, anexo_matematico.tex:243 | `paper_formal_firm_first_order_conditionsSpec` -> `paper_formal_firm_first_order_conditions` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
| Firma informal: PMgL, demanda de capital y beneficio residual / Section 4.2, Equation eq:PiI, anexo_matematico.tex:268 | `paper_informal_firm_profit_exhaustionSpec` -> `paper_informal_firm_profit_exhaustion` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
| Vaciamiento del mercado de bien formal (Ley de Walras) / Section 5.2, anexo_matematico.tex:302 | `paper_walras_formal_goods_marketSpec` -> `paper_walras_formal_goods_market` | not frozen | source-first, correcciones registradas | todas visibles en la firma | probado |
...
1. Intake seal, once the complete statement review is current: `python3
   scripts/semantic_audit_reuse.py --paper BN26InformalityWealthPeru --bootstrap-current
   --write`.
2. Proof loop: `lake build BN26InformalityWealthPeru.PaperInterface` or the narrower touched
   proof module.
3. Freeze closeout presentation inputs once: final report, status, source map,
   ...
   --paper BN26InformalityWealthPeru`; defer aggregate/site status to integration or release.
4. Closeout readiness: `python3 scripts/closeout_reuse_plan.py --paper BN26InformalityWealthPeru`.
...
- Current Lean endpoint: las ocho filas estan probadas; `lake build
  BN26InformalityWealthPeru` cierra sin `sorry`.
- Exact current mathematical gap: la existencia del equilibrio estacionario
  (solucion de HJB, distribucion invariante, y punto fijo de precios).
- Next bridge lemmas to try: para atacar la existencia haria falta primero una
  formalizacion del operador HJB discretizado y de la propiedad de M-matriz del
  esquema upwind, que es un proyecto separado.
- Informal proof sketch / recurrence / construction: no aplica en este momento.

## Deviations And Assumptions

- Source imprecision or proof deviation to report later: ninguna pendiente; las
  cinco correcciones estan en el expediente de fidelidad y en el reporte final.
- Genuine paper assumptions to declare in `Assumptions.lean`: ninguna.
- Temporary certificate fields to discharge: ninguno.
- Validation/audit checks that must inspect these assumptions: no aplica, no hay
